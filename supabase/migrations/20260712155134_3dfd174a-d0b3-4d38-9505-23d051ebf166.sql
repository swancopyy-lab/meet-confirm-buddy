...
-- Attach triggers to auth.users for profile/role creation
...
-- Backfill missing profiles + roles for all existing users
...
FROM auth.users u
LEFT JOIN public.user_roles r ON r.user_id = u.id AND r.role='host'
